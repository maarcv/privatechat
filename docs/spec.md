...
**Logging.** No log with content, names, keys, full `channel_id` or `pk`: only the 4-byte hex prefix when debugging is needed; in production, `warn` level and nothing else. All key material lives in the `Secret<N>` type (no `Clone`, no `Default`, manual `Debug` = `[REDACTED]`, `PartialEq` via `sodium_memcmp`). Redacted-`Debug` test (spec 010): `format!("{:?}")` of every type listed in `SECRET_TYPES` is exactly `[REDACTED]`. `core` emits no log in v1: it has no logging dependency, and spec 021-channel-session checks it. Log test (spec 100, with the server, the first crate that emits): in-memory `tracing` subscriber at TRACE level over a full flow with real clients, assert that neither hex nor base64 of `K_ch`, `sk_u`, `sk_ch`, `K_msg`, `K_hdr`, `mk` nor a full `channel_id`, `server_id` or IP appears in it.
...
Audits A (2026-09-19), B, C and D (2026-09-20), E, F, G, H and I (2026-09-24), the decisions taken while drafting phase 2 and audit J (2026-09-25): findings and applied changes are in `docs/audit-log.md`. Every PR that changes §3–§6 adds a row there.
