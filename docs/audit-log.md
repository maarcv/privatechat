...
| # | Finding | Severity | Change |
| --- | --- | --- | --- |
| E1 | CBOR through `ciborium` + `serde` brought about nine crates into `core`, the first external parser on the hostile path, and still needed a hand-written strict profile, integer-key serialiser, byte visitors and a two-step version read (A16, B11, C3, C14, C15, C16, 011-R17) | High | Own record encoding, `key` u8 ‖ `len` u32 BE ‖ `value`, strictly increasing keys, typed decoding, no dependency (ADR 0023, supersedes 0015); new spec 017-record-encoding; `core` keeps two dependencies (§4, §5, §6, §9) |
| E2 | `RetiredKey`, `PeerLimit` and `Replay` ran on an unauthenticated, bit-malleable header: the mutation table depended on receiver state and forged values could drive evictions, counters and false compromise alarms (A7, A8, B2, B9, C6, 013-R12) | High | Signature right after the header, before any state; no effect before it; every effect computed only for a consumed message (ADR 0024, §4) |
| E3 | Client expiry trusted the server's `received_at`: a malicious server could deliver an expired blob months later as new (B1) | High | A consumed message with `sent_at + ttl_ms + 360 000 < min(received_at, now)` is discarded as `Expired` (ADR 0024, §4, §6) |
| E4 | Phase 1 specs required the `Store`, cursor, peers, events and identity regeneration of phase 2, and had no pure whole-message entry point to fuzz or to generate vectors from (A2, A3, A5, A6, A14, C1, C5) | High | 013 defines pure `seal` (nonce as a parameter), `verify` and `open`; an authentic but unreadable message is `Ok(Unreadable)`; persistence moves to 021, 022, 024 and 025; phase 1 vectors carry no `commits` (specs 012–014, `specs/vectors/README.md`, §10 DoD) |
| E5 | 011–014 read vectors produced by 015, which depended on them (A4, C2) | High | 015 implemented right after 010; phase 1 order 010 → 015 → 017 → 011 → 012 → 013 → 014 → 016 (§10) |
| E6 | The 12 words needed SHA-256 for the BIP-39 checksum, absent from §4 and from spec 010 (A12, B13, C, 014-R12) | Medium | Words = first 132 bits of `fp` over the BIP-39 list, no checksum, not a mnemonic (ADR 0025, §4, §12) |
| E7 | `privatechat/fp/v1` is 17 bytes: the fingerprint input is 65 bytes, not 66 (A11, B3) | Medium | Corrected in spec 014; every spec asserts the length of its tags |
| E8 | The 7-word password had no owner and no byte encoding: cross-platform imports could fail, and UI generation put randomness outside libsodium (A15, B4, C12) | Medium | Generated by `core`, canonical bytes, canonicalised on open (ADR 0026, §5) |
| E9 | Optional invitation expiry with defaults, a `None` meaning never, and a stored config that could expire after import (A17, C11, C17) | Medium | Fixed expiry per form, no parameter; dropped once imported (ADR 0026, §5) |
| E10 | Raw binary config QR needs a different raw-byte scanner API per platform (C26) | Low | QR is base64url text of the record, no prefix, no scheme (ADR 0026, §5) |
| E11 | "No Cc/Cf" named no Unicode version: two app versions could disagree on whether a message is readable (B5, C15) | Medium | Only Cc (a fixed set) is checked; an invalid `display_name` is dropped, never makes a message unreadable; Cf removed on rendering (§4) |
| E12 | Derived vectors were produced by the implementation under test, so a derivation bug would be frozen as the reference (B6, C8) | Medium | Independent reference script with the Python standard library before freezing; exception in AGENTS 2 (spec 015, §11) |
| E13 | Vectors froze on first commit: a phase 1 bug would force `proto_version = 2` before v1 exists (C7) | Medium | Freeze at the phase 1 exit; before that, `adr-not-needed` plus the reviewer's reason (AGENTS 18) |
| E14 | u64 counters as JSON numbers do not survive Kotlin, Swift or 2^53-limited parsers (A18, C9) | Medium | 64-bit integers as BE64 hex in the vector files (`specs/vectors/README.md`) |
| E15 | The generator writing files contradicted AGENTS 4 and 10 literally, and "byte-identical" was checked once by hand (015-R5, C recommendation 3) | Low | Narrow exception named in AGENTS 4 and 10; an always-run test compares generated and committed files (spec 015) |
| E16 | Fuzz targets missed the post-signature path, `open_encrypted` and the verify QR; the fuzz crate cannot reach crate-internal functions; `arbitrary` unused (A20, A22, A23, B7, C3, C20) | Medium | `cfg(fuzzing)` entry module, signed-input and after-KDF targets, lint-table equality test, `libfuzzer-sys` only (spec 016) |
| E17 | No spec created `core::Error`; `OutOfMemory` from Argon2id could only surface as "wrong password" (C4) | Medium | Spec 011 creates the hand-written `Error` with a new `Internal` variant (§9) |
| E18 | The UI could build a `key_retired` or its own `sent_at` through a public `Payload` (C13) | Medium | `Channel::encrypt(body, display_name, now)`; `Payload` crate-internal (§4, §9) |
| E19 | `Config` exposed only `host()`: channels on one host and different ports would share a `Session`, and the UI could not open the socket (C10) | Medium | Accessors `server_url`, `suggested_name`, `ttl_seconds`; grouping by host and port (§9) |
| E20 | A decrypt verdict could leak through `on_frame` errors or reconnect timing (B16) | Low | No-oracle rule extended (§4) |
| E21 | `adr-guard` protected specs 011–013 only; the phase 1 CI line listed a log test no phase 1 spec carries (A26, A32) | Low | Guard extended to specs 011–014 and 017 (`ci.yml`, §10); log test deferred to spec 100 (§10) |
