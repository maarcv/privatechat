...
## Decision
`channel_id = BLAKE2b("privatechat/chid/v1" ‖ pk_ch ‖ BE32(ttl_seconds))[0..16]`. The TTL is part of the channel's identity: a different TTL is a different channel where nobody listens. The server recomputes `channel_id` from the `(pk_ch, ttl_seconds)` received in the signed subscription, checks the range, and stores `expires_at` for each `publish` (formula in `docs/spec.md` §6). There is no channel table; a channel exists while it has unexpired messages or subscribers.
...
- The client keeps enforcing its own expiry (`expires_local`, §6), ADR 0009.
