...
- The encoding is ours, so it is ours to get right. It is about 150 lines, it has its own spec, fuzz target and round-trip property test (specs 017-record-encoding and 016-fuzz-harness), and its vectors are cross-checked by the independent reference script of spec 015-test-vectors.
