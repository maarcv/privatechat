...
## Decision
1. **A single `Store`**, in Rust, in the `store` crate, shared by the three platforms. The trait has a single write operation, `commit(WriteBatch)`, atomic. Kotlin and Swift only unwrap the 32 B key, call `open_store(path, key_bytes)` once and zeroise their bytes; they never touch storage. `Channel` has no state that has not gone through `commit`. One directory, one process. The on-disk format is set by ADR 0021 (the initial version of this ADR said SQLCipher; replaced the same day).
