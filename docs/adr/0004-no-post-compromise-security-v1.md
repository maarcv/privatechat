...
## Decision
V1 does not attempt to recover security after a config leak nor to protect the messages prior to a leak. The response is key retirement (ADR 0016) and creating a new channel (ADR 0008) (Review C: ADR 0011 withdrawn). This is stated explicitly in the public documentation (`docs/spec.md` §1).
