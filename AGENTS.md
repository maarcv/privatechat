...
10. `core` does no I/O and never reads the clock: no `std::net`, `std::fs`, `tokio`, `reqwest`, `SystemTime::now`. Time comes in as a parameter. `cargo deny` (bans) and clippy `disallowed_methods` (`clippy.toml`, arrives with spec 010) enforce it.
...
18. The vectors in `specs/vectors/*.json` are regenerated only with a `proto_version` change and an ADR. CI (`adr-guard`) fails a diff that touches `specs/vectors/` without adding a new ADR file, unless a human sets the `adr-not-needed` label.
