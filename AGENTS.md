...
10. `core` does no I/O and never reads the clock: no `std::net`, `std::fs`, `tokio`, `reqwest`, `SystemTime::now`. Time comes in as a parameter. The single exception is the vector generator of spec 015 (`crates/core/src/vectors/generate.rs`), an ignored test that writes `specs/vectors/*.json`; nothing that ships reaches it. `cargo deny` (bans) and clippy `disallowed_methods` (`clippy.toml`, arrives with spec 010) enforce it.
11. Language: English for everything — identifiers, comments, error messages, test names, specs, ADRs, `docs/`, commit messages, pull requests and issues.
...
18. The vectors in `specs/vectors/*.json` freeze when phase 1 closes, after the reference script of spec 015 has cross-checked them; from then on they are regenerated only with a `proto_version` change and an ADR. Until then a vector change needs the `adr-not-needed` label and the reviewer's reason in the PR. CI (`adr-guard`) fails a diff that touches `specs/vectors/` without adding a new ADR file, unless a human sets that label.
19. No log with content, names, keys, full `channel_id` or full `pk`; at most the 4-byte hex prefix. The log test of spec 100 checks it, in whichever crate first emits.
