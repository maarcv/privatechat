...
10. `core` does no I/O and never reads the clock: no `std::net`, `std::fs`, `tokio`, `reqwest`, `SystemTime::now`. Time comes in as a parameter. The single exception is the vector generator of spec 015 (`crates/core/src/vectors/generate.rs`), an ignored test that writes `specs/vectors/*.json`; nothing that ships reaches it. `cargo deny` (bans) and clippy `disallowed_methods` (`clippy.toml`, spec 010) enforce it.
