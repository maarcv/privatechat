...
Blocks: 050-desktop-mvp, 053-device-security, 054-qr-invite, 055-verify-ui
...
- R6 Secrets and files cross as follows:
  ...
  - `export_qr` MUST render the config QR from `Device::export_qr` as an SVG image on the Rust side, with the `qrcode` crate and no default feature. The text and the SVG are held in `Zeroizing` buffers. The image is served on the custom URI scheme `qr` (`qr://localhost/<n>`, and `http://qr.localhost/<n>` on Windows), where `<n>` is a counter, with `Cache-Control: no-store` and no CORS header. `export_qr` returns only that URL. The URL answers until the next `export_qr`, a `leave` of that channel, `lock`, or 60 000 ms after `export_qr` returned, whichever comes first (the display time of spec 054-qr-invite). `export_verify_qr(channel)` renders the `qr` of `own_fingerprint` the same way on the same scheme (spec 055-verify-ui), with no expiry but `leave` and `lock`, since that QR is public. The page can show the image, but no script can read its pixels (a cross-origin image taints a canvas, measured in WKWebView on 2026-09-26) or fetch it (R8's `connect-src`). The text never enters the web view.
  - The open and save dialogs are opened from Rust with `tauri-plugin-dialog`, through `Dialogs` and under the rules of R9, so that the bytes of a `.chatcfg` file never enter the web view.
...
- R7 The host MUST take the events out of every `Device` result that carries them: `on_frame`, `on_tick`, the `events` of `Sent`, and the result of `regenerate_identity`. It acts on `Reconnect`, `ServerFull` and `UnsupportedServer` as R13 says, and emits every other event to the web view as `core-event`: a tagged union of the `Event` variants of spec 028-session-sans-io that name a channel (`Subscribed`, `HistoryTruncated`, `Message`, `Delivered`, `NotDelivered`, `StatusChanged`, `StorageFailed`, `SubscribeRefused`, `ChannelFull`), typed by R5. `send` therefore returns `Sent` without `events`, and `regenerate_identity` returns nothing. The host MUST also emit `connection-state { channels, state }` for the channels of a plan whenever their state changes, with `state` one of:
  ...
  - `failed`, for a plan kept out of reconcile after a panic (R10), until the recorded channels it holds are left or the process restarts.
...
- R9 The host MUST NOT depend on the Tauri runtime. It emits through a trait `Sink`, and opens dialogs through a trait `Dialogs`: `open_file`, `save_file` and `confirm`, each an async call, and `main_window_focused`. The app implements both over Tauri's `emit`, `tauri-plugin-dialog` (file dialogs) and `rfd` (confirmations), and the tests over channels and scripted answers, so that the exit test (R18) runs the host without a window. The host, not `Dialogs`, applies every rule below before it calls `Dialogs`, and measures every delay on the `tokio` clock, so that tests can pause it. Every dialog, file dialogs included:
  - is one at a time: a request while one is open returns `Cancelled`;
  - opens only while the main window is focused, otherwise `Cancelled`, and the host calls `Dialogs::focus_main_window` once it closes;
  - opens no sooner than 1 000 ms after the previous dialog closed (`Suppressed`), so that a quit click can land between dialogs a script chains;
  - is refused with `Cancelled` once the app is quitting (R17).
  A confirmation MUST also:
  - be an `rfd::AsyncMessageDialog` (pinned, `default-features = false`, features `gtk3` and `common-controls-v6`, so that zenity never runs) with `rfd::MessageButtons::OkCancelCustom(<safe label>, <action label>)`, so that the safe button comes first and is the default. On macOS and Windows its parent is the main window, so that it is a sheet or an owned dialog of the app that cannot hide behind it and goes away with the process (measured on macOS on 2026-09-26: a parentless dialog there is drawn by another process, outlives the app, and can keep the slot taken for good); on Linux, where rfd's GTK dialog takes no parent, it is the synchronous `MessageDialog::show` inside `spawn_blocking`. It never runs on, or blocks, the main thread (measured: a synchronous dialog run from the main thread stops the event loop on macOS). Only rfd's own `Custom(<action label>)` accepts; `Cancel`, a dismissal and any other answer are a decline. `tauri-plugin-dialog`'s message dialog MUST NOT be used for a confirmation, since it maps a dismissal to the second label (measured);
  - show the same dialog again at once when an accept returns less than 1 000 ms after the host called `show`, keeping the slot and with no quiet period, no suppression and no focus check (it is the dialog the user just answered; `quitting` is still checked), so that a click the page timed to land on the button is never a yes and an honest double-click costs nothing (the input protection of browsers' permission prompts);
  - after the user declines, return `Suppressed` with no dialog for the same kind for 60 000 ms, so that a script cannot repeat a dialog until it is accepted. `Unlock` is never suppressed after a decline (the 1 000 ms quiet period still applies to it): a script that raises it gains nothing unless the user, who wants to unlock, accepts;
  ...
  A save path that returns less than 1 000 ms after the save panel was shown is `Cancelled`, with nothing written and no `Device` call, so that an Enter meant for the page does not export a channel. While the device is locked, every command other than `unlock`, `lock` and `reset_local_data` returns `Locked` before any dialog.
...
- R16 The storage key MUST live in the operating system's keychain through `keyring` 4, with the native store of each system (the macOS Keychain; the Windows Credential Manager; the Secret Service over D-Bus on Linux). It is one entry whose service is the app identifier constant and whose user is `storage-key:` followed by the canonical path of `app_local_data_dir()` (`fs::canonicalize`; the app creates that directory at start for `instance.lock`) and `/data`, so that it can be named whether the data directory exists or not, so that an install at another path (another packaging, another `$XDG_DATA_HOME`, a development build) never reads or deletes this one's key, and two spellings of one directory reach the same entry. On Windows a user longer than 513 characters (`CRED_MAX_USERNAME_LENGTH`) makes `unlock` return `FileIo`. The entry holds 32 bytes read and written with `get_secret` and `set_secret`, in `spawn_blocking`, and every read returns a `Zeroizing` buffer. The host calls `keyring::Entry::store_status()` once, before any entry is built, and maps its error to `KeychainUnavailable`. On Windows the entry MUST be built through `keyring-core` (pinned at the exact version that `keyring` locks) with `Entry::new_with_modifiers` and the persistence `Local` (`CRED_PERSIST_LOCAL_MACHINE`), so that it does not roam; keyring 4's own `Entry::new` passes no modifier and the Windows store defaults to `Enterprise`, which roams, while the other stores reject that modifier and use `keyring::Entry::new`. `unlock` while unlocked MUST return `Ok` without reopening, with no dialog and no suppression, before anything else but the quitting check of R17. While this process has opened no `Device` yet, `unlock` opens with no dialog; once it has opened one, every later `unlock`, whatever locked the device (`lock`, a reset, spec 053-device-security), MUST first ask through a confirmation (R9), since reading the keychain asks the user for nothing and a script that outlived the lock could otherwise unlock at once (decided with the human reviewer, `docs/audit-log.md`, "Audit L", L-Q8). Otherwise `unlock` MUST behave as follows:
  ...
  - `reset_local_data`. It then locks when unlocked (R17), deletes the data directory, and only then the keychain entry; the current window's web view directory is left to the quit path and the next start (R8). When the data directory cannot be fully deleted, it returns `FileIo` and keeps the entry. The next `unlock` then starts as a fresh install.
...
- R17 The lock routine (R15) MUST, in this order:
  ...
  - stop the tick;
  - abort every plan task and wait for each to end, with the lock released, then call `on_disconnect` for each plan that was connected, in one hold of the lock; when the lock is poisoned, skip this call and the next;
  - call `Device::flush(now)`, and drop the `Device` whatever `flush` returned; a poisoned lock is recovered only to drop the `Device`, with no call on it, and then cleared;
  ...
  - On macOS the app replaces the default menu, whose Quit item is `terminate:` and skips `ExitRequested`, with one whose Quit is a plain item with the accelerator `CmdOrCtrl+Q` that starts this path (measured on 2026-09-26: the handler runs and `terminate:` does not), and which keeps an Edit submenu of the predefined items, so that copy and paste still work in the web view. A termination that bypasses it (the Dock's Quit, logout) reaches only `RunEvent::Exit`, where the host drops the `Device` synchronously, best effort, without `flush`, and only when `try_lock` gets the lock at once: a documented residual. A close or quit while an `unlock` waits on a system keychain prompt takes effect once that prompt is answered. When the device locks on its own (session lock, timeout) is spec 053-device-security's.

**The exit test**

- R18 `clients/desktop/src-tauri/tests/phase4_exit.rs` MUST run two hosts, each over its own temporary data directory, an in-memory keychain store and `Dialogs` that report the main window focused and confirm 1 000 ms after they are shown (real time, inside the 120 s budget). They run against the `privatechat-server` binary of spec 035-server-ops, whose path the CI job passes in `DESKTOP_TEST_SERVER_BIN`. The test starts it with an empty environment (`env_clear`) plus exactly:
...
| Dialogs | one at a time, file dialogs included; only while the main window is focused; none for 1 000 ms after any closes; none of a kind for 60 000 ms after a decline, `Unlock` excepted | `Cancelled`; `Suppressed` |
| An accept or a save path | ≥ 1 000 ms after the host showed the dialog | shown again; for a save, `Cancelled` |
...
pub trait Dialogs: Send + Sync + 'static {   // the host applies R9's rules before each call
    fn open_file(&self) -> BoxFuture<'static, Option<PathBuf>>;
    fn save_file(&self) -> BoxFuture<'static, Option<PathBuf>>;
    fn confirm(&self, what: Confirmation) -> BoxFuture<'static, bool>;
...
impl Host {
    ...
    pub async fn export_verify_qr(&self, channel: [u8; 16]) -> Result<String, BridgeError>;          // the qr: URL of own_fingerprint's qr
...
- A panic inside the core is a bug, but a server that finds one can trigger it; the plan it came through is kept `failed` after the next unlock (R10), so the user can leave its channels. A panic in `on_tick` or a command, for instance from stored state, locks the device again at every unlock, and only `reset_local_data` gets out of it. Both are documented residuals.
- On Linux the confirmation has no parent and can fall behind the main window while it holds the dialog slot; the UI of spec 050-desktop-mvp says "answer the system dialog" while a command that can open one is pending.
...
- T09 (covers R9): `s041_t09_r09_host_and_dialogs`, with a paused `tokio` clock and scripted `Dialogs`:
  - `Host` is built and unlocked in a plain `tokio` test with a channel `Sink`.
  - A second dialog while one is open → `Cancelled`; any dialog while `main_window_focused` is false → `Cancelled`; any dialog within 1 000 ms of another closing → `Suppressed`; a file dialog while a confirmation is open → `Cancelled`.
  - An accept 10 ms after `show` shows the same dialog again, with no suppression, even while `main_window_focused` still reads false; after every dialog the host calls `focus_main_window`; a save path 10 ms after the panel → `Cancelled` and nothing written.
  - A declined `reset_local_data` makes a second one within 60 000 ms return `Suppressed` with no dialog; a declined `Unlock` does not.
  - Each backend's dismissal result (rfd `Cancel`) and a `Custom` of the safe label map to a decline; a main-thread closure and an emit still run while a confirmation is open.
  - A `SetProxy` for `127.0.0.1.nip.io:1080` is shown as "on another computer"; `remove_broken` of an unknown name → `UnknownChannel` with no dialog; each failed re-check gives the result R9 names.
  - Every locale of `src-tauri/strings/` has the English keys and placeholders, distinct action labels, and no `&` or `_`.
  - Ticks keep their 1 000 ms rhythm while a confirmation is open; `lock` during an open confirmation, a `reset_local_data` one included, ends at once, and a later yes of any but the reset and the unlock returns `Locked` and changes nothing; while locked, `set_socks5_proxy` returns `Locked` with no dialog.
...
- T16 (covers R16), `s041_t16_r16_unlock`, over the in-memory `KeyStore` unless stated:
  ...
  - `unlock` twice at start → one open, and both return `Ok`; an `unlock` queued behind a `lock` that follows the first open, with no confirmation asked → `Cancelled`, still locked; after a successful reset, `unlock` asks nothing and opens an empty device; `lock` during an `unlock` → locked once the `unlock` ends; the first `unlock` of the process asks nothing, and one after a `lock` asks `Unlock`, declined → `Cancelled` and still locked.
...
- on macOS, a clipboard read inside a click handler shows the "Paste" callout of the Security section and nothing more;
- before PR slice (c), a measurement on Linux that rfd's synchronous GTK dialog inside `spawn_blocking` runs (rfd drives GTK on a thread of its own); if it does not, R9 is amended to run it on the main thread, where GTK nests its loop;
- on each of macOS, Windows and Linux, Esc and the close gesture on a confirmation never give `Custom(<action label>)`; on macOS and Windows the confirmation cannot be put behind the main window and goes away when the app quits; on Linux the synchronous dialog inside `spawn_blocking` opens and answers without stopping the event loop.
...
- 2026-09-26 revised after audit L round 6 (`docs/audit-log.md`): confirmations parented to the main window on macOS and Windows; a too-fast accept shows the dialog again; `Unlock` never suppressed; the quiet period 1 000 ms and the focus rule for every dialog; `Dialogs` async with a focus query; the poisoned lock routine spawned and generation-tagged, with no call on a poisoned `Device`; the failing plan kept out; reset resets the open count and leaves the live web view directory; the Edit menu kept; the exit test's dialogs made to pass
- 2026-09-26 revised after audit L round 7 (`docs/audit-log.md`): the failed plan recorded by its channels; `focus_main_window`; the quick re-show skips the focus check; the unlock check order; the Linux dialog measured before slice (c)
- 2026-09-26 amended while drafting phase 5 (`docs/audit-log.md`, "Phase 5 drafts"): `export_verify_qr` for spec 055-verify-ui, and the invitation image's 60 000 ms lifetime for spec 054-qr-invite (R4, R6, Interface)
