...
- The client never resumes a TLS session (§6, specs 050–052), so a session ticket a proxy issues is never used. nginx disables them anyway (R4). Caddy keeps its tickets on, unused, as decided for 034-R3.

## Public API changes

None in `core`. `docs/spec.md` §9 "Server" and §11 `deploy/` already name these files. At acceptance, `docs/spec.md` §6 "Operation" says `HiddenServicePort 80` (ADR 0038) instead of 443 names the proxies' error-log settings, and says that "no session tickets" holds for nginx while Caddy's stay on unused (034-R3), and `.github/CONTRIBUTING.md` gains the commands of the job `docker`.
...
Decided with the human reviewer on 2026-09-25: 034-R3, Caddy's session tickets. Caddy can turn them off only in its JSON configuration (`apps.tls.session_tickets.disabled`), not in the Caddyfile. The reference stays the Caddyfile, tickets on, and `deploy/README.md` says they are unused: the clients never resume a session (§6, specs 050–052), so a ticket is never presented and links nothing. nginx keeps `ssl_session_tickets off` (R4).
