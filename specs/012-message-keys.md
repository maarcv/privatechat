...
Status: in review
Phase: 1
Related ADRs: 0002, 0013, 0018, 0019, 0024
Depends on: 010-primitives-wrapper, 011-config-format, 015-test-vectors
Blocks: 013-wire-message, 021-channel-session, 022-peers-tofu
...
- R7 `check_counter` MUST return `Error::Replay` for a counter lower than or equal to `max_counter` and `Ok` for a counter above it; it MUST be called only on a header whose signature has been verified, and the caller MUST advance `max_counter` to the counter only once the message is consumed (`docs/spec.md` §4 step 7).
- R8 With `max_counter = None` (an unknown key, or one evicted), `check_counter` MUST accept any counter, including 0 and 2^64 − 1.
- R9 `gap` MUST return `missing = counter.saturating_sub(max_counter + 1)` for `Some(max_counter)`, computed without overflow, and `missing = 0` for `None`; `anomalous` MUST be true exactly when `missing > 2^32`. It MUST be evaluated only for a consumed message (`docs/spec.md` §4).
...
- R11 `own_key_verdict(send_counter, counter)` MUST return `OwnKey::Echo` when `counter < send_counter`, which the caller reports as `Error::Replay`, and `OwnKey::UsedElsewhere { next_send_counter }` otherwise, with `Some(counter + 1)` for `counter` below 2^64 − 1 and `None` (the send counter is exhausted) for `counter = 2^64 − 1` (`docs/spec.md` §4 "Messages from one's own key").
- R12 Every function of this spec MUST be a pure function of its inputs: no clock, no randomness, no state. The only randomness of a message is its nonce, which the caller of spec 013-wire-message draws.
- R13 `K_msg`, `K_hdr` and `mk` MUST be `Secret<32>`, which introduces no new secret type, and every item of this spec MUST be `pub(crate)`, so that none of them can reach a caller outside `core`.
- R14 The two 8-byte contexts MUST be named constants next to the code that uses them, equal byte for byte to the literals of `docs/spec.md` §4 and exactly 8 bytes long; changing any of them MUST require a new `proto_version` and an ADR (AGENTS 3).
...
impl ChannelKeys {
    pub(crate) fn derive(channel_key: &Secret<32>) -> Result<ChannelKeys, CryptoError>;
}
...
pub(crate) fn message_key(keys: &ChannelKeys, sender_pk: &PublicKey, counter: u64) -> Result<Secret<32>, CryptoError>;

pub(crate) struct Header { pub(crate) sender_pk: PublicKey, pub(crate) counter: u64 }

impl Header {
    pub(crate) fn seal(&self, keys: &ChannelKeys, nonce: &Nonce) -> Result<[u8; ENC_HDR_LEN], CryptoError>;
    pub(crate) fn open(sealed: &[u8; ENC_HDR_LEN], keys: &ChannelKeys, nonce: &Nonce) -> Result<Header, CryptoError>;
}

pub(crate) struct Gap { pub(crate) missing: u64, pub(crate) anomalous: bool }
pub(crate) enum OwnKey { Echo, UsedElsewhere { next_send_counter: Option<u64> } }

pub(crate) fn check_counter(max_counter: Option<u64>, counter: u64) -> Result<(), Error>;
pub(crate) fn gap(max_counter: Option<u64>, counter: u64) -> Gap;
pub(crate) fn next_send_counter(current: u64) -> Result<u64, Error>;
pub(crate) fn own_key_verdict(send_counter: u64, counter: u64) -> OwnKey;
```

`Header::open` returns a `Header` and never an error of its own (R5): a wrong key is indistinguishable from a right one until the signature is checked. Its `CryptoError` can only be `InitFailed`, which `core::Error` maps to `Internal`.
...
- The header has no integrity of its own: anyone can flip bits of `enc_hdr` without a key. This is why every decision function here runs only after the signature has been verified (R7, R9, ADR 0024): a forged counter can neither trigger `Replay` for a real sender, nor raise a false anomalous jump, nor move one's own send counter.
...
- T02 (covers R2): `s012_t02_r02_message_key_known_answer` on the vectors `message_key_*`; `s012_t03_r02_message_key_changes_with_every_input`: a different `pk_u`, or a counter that differs by one, gives a different `mk`.
- T04 (covers R3): `s012_t04_r03_message_key_is_never_stored`: the sources of `proto/` declare no field of type `Secret<32>` outside `ChannelKeys`, and `message_key` is called only from the seal and open functions of spec 013-wire-message. `ZeroizeOnDrop` of `Secret` is already proven by spec 010-primitives-wrapper.
...
- T13 (covers R11): `s012_t13_r11_own_key_verdict_table` on the vector `own_key_table`: a counter below, equal to and above the send counter, and `counter = 2^64 − 1` → `UsedElsewhere { next_send_counter: None }`.
- T14 (covers R12): `s012_t14_r12_functions_are_pure`: the same inputs give the same outputs a thousand times, and no function takes a `now`.
- T15 (covers R13): `s012_t15_r13_no_new_secret_type`: `SECRET_TYPES` is unchanged by this spec and no item of `proto/keys.rs`, `proto/header.rs` or `proto/replay.rs` is `pub`.
- T16 (covers R14): `s012_t16_r14_contexts_are_the_literals`: the constants are byte for byte the ones of `docs/spec.md` §4 and each is 8 bytes long.
- T17 (covers R1, R2, R4, R7, R9, R11): `s012_t17_r01_every_vector_is_checked`: a loop over every vector of `012.json` with one match arm per name and no wildcard, so an unknown or unused vector fails (spec 015-test-vectors).

## Vectors

`specs/vectors/012.json`, schema of `specs/vectors/README.md`, `proto_version = 1`. Every 64-bit value (counters) is a 16-character big-endian hex string. Produced by the generator of spec 015-test-vectors, cross-checked by its reference script, and frozen when phase 1 closes (AGENTS 18).
...
| `own_key_table` | positive | derived | the pairs `(send_counter, counter)` of T13 → verdict |

The vectors of this spec are `derived`: every value follows from the formulas of `docs/spec.md` §4 and the primitives of spec 010-primitives-wrapper. `master_keys` and `message_key_*` are BLAKE2b computations the reference script of spec 015-test-vectors recomputes with the Python standard library; the header uses XChaCha20, already covered by the published vectors of spec 010-primitives-wrapper.
...
- Where `max_counter`, the send counter and the peers live, the commit that advances them, the `OwnKeyUsedElsewhere` event and the read-only state after one's own `key_retired` (specs 021-channel-session, 022-peers-tofu, 024-key-retired).
