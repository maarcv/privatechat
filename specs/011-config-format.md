...
- T05 (covers R5, R6): `s011_t05_r05_url_grammar` over a table of accepted and rejected URLs, including `wss://host`, `wss://host:443`, `wss://1.2.3.4`, `wss://[::1]:9001`, a 56-character `.onion`, and the rejections `ws://host`, `wss://host/path`, `wss://host?q`, `wss://HOST`, `wss://host.`, `wss://host:0`, `wss://host:065`, `wss://user@host` and a 257-byte URL; `s011_t06_r06_host_has_no_brackets_or_port` checks `host()` for the IPv6 and the port cases.
...
- [ ] 011-R5: the 256-byte maximum for `server_url` and the whitelist grammar are this spec's, not `docs/spec.md`'s. They exclude a Unicode host and any path, which no current use needs. Confirm both numbers.
- [ ] 011-R14: the 256-byte maximum for the password is this spec's. The app generates 7 BIP-39 words, at most 62 bytes, so the margin is for a password typed or pasted by hand in a future version. Confirm.
