...
- R4 `fuzz_entry::record_decode` MUST drive a fuzz-only schema that uses every type of `docs/spec.md` §4 "Record encoding" — `u8`, `u32`, `u64`, `bool`, `bytes`, `bytesN`, `text`, a nested record and a `list` — under the unknown-key policy chosen by the first input byte, so that the nested and list paths are fuzzed before any phase 2 schema uses them.
...
- R11 Every encoder and decoder pair MUST have a `proptest` round-trip over the full range of its limits. The eight pairs are the record, the config record, the config QR text, the config file through its fixed-key internal open, the payload, the envelope (`seal`, then `verify` and `open`), the padding, and the verification QR.
- R12 A target MUST NOT depend on a clock, on randomness or on the file system: the fuzzer supplies every byte, including the times of R5.
...
The exclusion list of R7 starts with `Config::export_encrypted` (it takes no external bytes since ADR 0028, and seals rather than parses) and the password canonicalisation of spec 011-config-format (a pure string transform covered by its own property test), each with that reason.
...
- The `.chatcfg` file is not fuzzed past Argon2id: a mutated file almost never passes the `secretbox` tag, so a fixed-key target would only reach the three header comparisons its own tests already cover. The file is covered by its round-trip through the fixed-key internal open (R11).
...
- T11 (covers R11): `s016_t11_r11_round_trips`: the eight `proptest` round-trips, each over the full range of its limits.
- T12 (covers R12): `s016_t12_r12_targets_are_pure`: no target or `fuzz_entry` function names a clock, a random source or the file system.
...
- Fuzzing past Argon2id, and a round-trip target that repeats spec 013-wire-message's property test.
