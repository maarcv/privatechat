...
- R8 "Send invitation file" MUST first show a confirmation with the warning of R4 and: "Send the file by any route, and tell the seven words by another route, for instance in a call. Never send them in the same message as the file. The file can be opened until HH:MM tomorrow." Only after it does the client call `export_file`. On the desktop, spec 041-desktop-bridge R6 opens the save dialog first, then exports, writes, and shows the words. On Android and iOS, no secret is produced before a system screen the app opens returns (spec 053-device-security R8, `docs/audit-log.md`, "Audit M", M-Q1):
  - "Save to a file": the destination is chosen first (Android `ACTION_CREATE_DOCUMENT`; iOS a folder chosen with `UIDocumentPickerViewController(forOpeningContentTypes: [.folder])`, since `fileExporter` takes the document when it is presented); on its return, `export_file`, the file written to the chosen place (on iOS inside the folder's security-scoped access), then the words (R10);
  - "Share": `export_file`, the file written to the temporary file of R9, the words (R10) and the user's "I have told them the words" or "I have written them down"; only then the share sheet opens, with the words already gone. While the temporary file exists, the screen offers "Share again", which opens the share sheet with the same file and draws no new password.
- R9 On Android and iOS the file MUST be written, 1 085 bytes, with the name `invitation.chatcfg`, which never carries the channel's name, either to the place the user chose or to a temporary file in the app's cache directory handed to the system share sheet:
  ...
  The temporary file MUST be deleted when the iOS completion handler runs; 600 000 ms after the share sheet opened, when the app is in the foreground at that moment; at the next unlock, only when the file is older than 600 000 ms; and at the launcher activity's or scene's first start after that age. It is never deleted at the lock that the share sheet itself causes, nor at a process start or a `FileProvider` start, since the target app may still be reading it (Android starts the process to serve the provider).
- R10 The seven words MUST be shown once, numbered 1–7, on a full screen that follows a successful export, from the byte array the export returned. They are not selectable and no action copies or shares them. The screen MUST say: "Tell these seven words by another route. Anyone with the file and the words reads the channel. Leaving the app erases them: write them down or tell them now." The array MUST be overwritten with zeros, and the screen dropped, when the user leaves it, when the app goes to the background or locks, when the desktop's main window loses focus, and on an iOS screenshot, which also shows: "A screenshot of these words is in your Photos. Delete it. The words alone do not open the channel; if they left this phone together with the file, create a new channel.", with the "Create new channel" action of R3. There is no way to show the same words again: a new export draws a new password (spec 011-config-format R16).
...
- R12 The scanner of this spec MUST be the one component, per platform, that reads a QR from the camera, and spec 055-verify-ui MUST reuse it with its purpose:
  - It takes a `purpose`, `Invite` or `Verify`, which sets its texts, and an accept predicate, the caller's shape check (for `Verify`, the prefix `verify:v1:` and the length of spec 055-verify-ui R19; for `Invite`, any payload under the rule below). A payload the predicate refuses is overwritten with zeros by the scanner, its caller shows the refusal text, and scanning goes on.
  ...
  - A payload is delivered only once it has been decoded in 3 consecutive frames, each holding that one QR and no other. When two different payloads have been seen since the scanner opened, it delivers nothing and shows "More than one code in view: scan only the other person's screen.", until it is closed and opened again. This reduces the chance that a code planted next to the real one is taken; the defence is the confirmation of R15, which shows what would be joined.
  - The scanner delivers one accepted payload to its caller, then stops the camera. It never interprets, opens or follows the text: it is not a URL handler, and it offers no import from saved pictures.
  - Its caller MUST overwrite the payload with zeros on every exit: after its last core call, or at once when it makes none (a refusal of its own, cancel, leaving the screen, background, lock), whatever the purpose and the result, since a payload scanned by mistake may be an invitation. Each core call receives a fresh copy of the kept bytes, since `Core` zeroes what it is given (spec 040-uniffi R9).
...
- R15 An import MUST NOT join before the user confirms it (decided with the human reviewer, `docs/audit-log.md`, "Audit M", M-Q4). The client first calls `preview_qr(text, now)` or `preview_file(bytes, password, now)`, which parse the invitation and commit nothing (R17). The confirmation screen then shows:
  ...
  - "Onion servers need the Tor proxy set in the settings." when the preview says `needs_proxy` (R17), the text of R2; "Join" stays available, and the channel connects once a proxy is set (spec 027-core-api R10).
  When the preview says `is_open`, the screen shows "You already have this channel." with its "Open" action, and no "Join" and no editable name. Otherwise "Join" is enabled only while the name is 1..=64 bytes with no Cc character, the rule of R1. Only "Join" calls `import_qr` or `import_file`, with a fresh copy of the kept bytes and password, and then, when the user edited the name, `rename_channel` with it; a failed `rename_channel` after a successful import shows "Joined; the name could not be changed." and opens the channel. "Cancel" calls nothing. The client keeps the text, the file bytes and the password only while this screen, and the replace confirmation of R16, are open, and zeroes them when they close, after the last import call, and when the app locks; on Android and iOS also when the app goes to the background. On the desktop focus loss does not drop them: the focus timer of spec 053-device-security R8 bounds them through the lock.
...
- R17 This spec amends spec 027-core-api with two functions of `Device` that commit nothing, and specs 040-uniffi and 041-desktop-bridge with their twins and commands:
  ...
  - `ConfigPreview { suggested_name: String, server_url: String, ttl_seconds: u32, is_open: bool, server_known: bool, name_collides: bool, needs_proxy: bool }`, where `is_open` says a channel with that `channel_id` is open, `server_known` that an open channel has that `server_url`, `name_collides` that the `name_key` of the suggested name (spec 022-peers-tofu R4) equals the key of an open channel's `local_name` or `suggested_name`, and `needs_proxy` that the channel would be in no plan with the current settings (spec 027-core-api R10). The collision is computed by the core, since the UI holds no logic (architecture skill §7).
  `preview_file` runs the password derivation, so a file import costs it twice, about one second each. In the pull request that marks this spec `accepted`:
  - spec 027-core-api MUST gain the two functions and the record above, in a requirement and its Interface;
  - spec 040-uniffi MUST gain `previewQr`, `previewFile` and `FfiConfigPreview`, and R7 and R9 MUST zero the arguments of `preview_qr` and `preview_file` as they do those of the imports;
  - spec 041-desktop-bridge MUST drop the `import_qr` command from R4, R6, T04 and T06 (the host method stays for R18's exit test) and the sentence of its Security on "a QR text the user pastes to import"; gain the command `preview_file` over the pending file with the password as a raw body, in R4, R6's raw-body rule and T04; and in R6 build both `export_qr` and `export_verify_qr` with the `qrcode` crate at `default-features = false, features = ["svg"]`, from one segment written with `Bits::push_byte_data` at level M in the smallest version from 1 to 40 that fits, then `QrCode::with_bits` (measured on 2026-09-26: the versions equal the automatic encoder's, and ZXing and Vision decode them);
  - the architecture skill §7 and the typescript-svelte skill MUST replace "passed once, then filled with zeros" by "each core call gets a fresh copy; the kept copy is filled with zeros on every exit".

## Limits

| Input | Range | Out of range |
| --- | --- | --- |
| Channel name | 1..=64 B UTF-8, no Cc | refused in the form; "Join" disabled |
...
| Scanned payload | 1..=700 printable ASCII characters; the same one QR in 3 consecutive frames; one payload per scanning session | ignored, scanning goes on; two payloads: nothing until reopened |
| File read (mobile) | ≤ 1 085 B; at most 1 086 B read | `BadConfig` message |
| Typed password | ≤ 1 024 B | refused in the field |
| Temporary export file | until the iOS completion handler, 600 000 ms after the share in the foreground, or the first unlock or start after that age | deleted |
...
clients/fixtures/qr/
  one PNG per renderer (ZXing QRCodeWriter, CIQRCodeGenerator, the desktop's qrcode SVG rasterised) of 011 config_reference and 014 qr_reference (T05)
...
InviteState = Warning(kind: Qr | File) | ChoosingDestination | ShowingQr(bitmap, secondsLeft, expiresAt) | Exporting
            | ShowingWords(words, expiresAt) | Sharing | Shared(canShareAgain) | Closed
InviteIntent = Confirm | Dismiss | Tick | Leave | ToldThem | WroteThemDown | Destination(uri) | ShareAgain
ImportState = Choosing | Scanning(permission, conflict) | EnteringPassword(fileUri) | Previewing | Confirming(ConfigPreview, name, canJoin)
            | AlreadyHave(ChannelId) | Importing | ConfirmReplace(reason) | Done(ChannelId, isNew, renamed) | Failed(ImportFailure)
...
fun interface QrScanned { fun onPayload(bytes: ByteArray) }   // an accepted payload, once, then the camera stops; the caller zeroes it on every exit
fun interface QrAccept { fun accepts(bytes: ByteArray): Boolean }   // the caller's shape check; a refused payload is zeroed by the scanner
@Composable fun QrScannerView(purpose: ScanPurpose, accept: QrAccept, onScanned: QrScanned, onRefused: () -> Unit, onPermissionDenied: () -> Unit)
```
...
struct QRScannerView: UIViewControllerRepresentable {
    ...
    let accept: (Data) -> Bool                              // the caller's shape check; a refused payload is zeroed by the scanner
    let onPayload: @MainActor (Data) -> Void               // an accepted payload, once, then the session stops; the caller zeroes it on every exit
...
pub struct ConfigPreview { pub suggested_name: String, pub server_url: String, pub ttl_seconds: u32, pub is_open: bool, pub server_known: bool, pub name_collides: bool, pub needs_proxy: bool }
impl Device {
...
- The scanner never keeps a frame and never acts on the text it reads (R12); what it reads goes to the core, which decides, and nothing joins before the user has seen the name, the server and the lifetime (R15). Two codes seen in one scanning session give nothing, and a payload must hold for three frames, which reduces the chance that a planted code is taken; the defence is the confirmation, where a look-alike channel name or a server new to the device is flagged (R12, R15, R17).
...
Two functions and one record are added to the core by R17: `Device::preview_qr`, `Device::preview_file` and `ConfigPreview`, with their twins in spec 040-uniffi (`previewQr`, `previewFile`, `FfiConfigPreview`) and their commands in spec 041-desktop-bridge (`preview_file` over the pending file and a raw password body), together with the removal of the desktop's `import_qr` command and the single byte segment of its QR images. Specs 027, 040 and 041 are amended when this spec is accepted (R17).
...
- T08 (covers R8): `s054_t08_r08_file_order`: no `export_file` before the warning's action; on mobile, "Save to a file" opens the picker (on iOS the folder picker) before `export_file`, and "Share" opens the share sheet only after the words are gone; "Share again" reopens the sheet with the same file and makes no second `export_file`.
- T09 (covers R9): `s054_t09_r09_temporary_file`: the file is 1 085 bytes and named `invitation.chatcfg`; the Android intent has no `EXTRA_TEXT` or `EXTRA_SUBJECT` and the label `invitation`; the iOS sheet excludes the three activity types; the temporary file survives the lock the share causes, an unlock within 600 000 ms and a provider-only process start, and is gone after the completion handler, after 600 000 ms in the foreground, and at the first unlock or launcher start after that age; the `FileProvider` is not exported.
- T10 (covers R10): `s054_t10_r10_words_once`: seven numbered words on a full screen; no copy action; the array is all zeros after leaving, background, lock and desktop focus loss; the screen cannot be reopened with the same words; an iOS screenshot of the words shows the words' own warning.
...
- T12 (covers R12): `s054_t12_r12_scanner`: a fake camera feed with a QR of 011 `config_reference` in 3 consecutive frames delivers its bytes once and stops, and in 2 frames delivers nothing; a QR of 701 characters and a non-ASCII text deliver nothing; a frame with two QRs, and two different QRs in successive frames, deliver nothing and show the "More than one code" text; a payload the accept predicate refuses is zeros and scanning goes on; a QR holding `https://example.org/` is delivered as bytes and opens nothing; the payload is all zeros after its caller's last call, and after a cancel, a leave, a background and a lock with no call, for each purpose; a refused permission shows the text of each purpose and the settings action; no frame is written to disk.
...
- T15 (covers R15): `s054_t15_r15_confirm_before_join`: no `import_qr` or `import_file` before "Join"; "Cancel" calls nothing and zeroes the kept bytes; an edited name is passed to `rename_channel`; an empty name or one of 65 bytes disables "Join"; a failed rename after joining shows its text and opens the channel; `is_open` shows "You already have this channel." with no "Join"; a server absent from `channels()` shows the new-server mark; `name_collides` and `needs_proxy` show their texts; each core call gets a fresh copy, so the replace retry receives the full bytes; the kept bytes are zeros after the last call, on mobile after background, and on the desktop after lock but not after focus loss.
