...
Blocks: 011-config-format, 012-message-keys, 013-wire-message, 014-fingerprint, 015-test-vectors, 016-fuzz-harness, 017-record-encoding, 020-store-files, 022-peers-tofu, 031-auth-channel-signature, 032-storage-ttl, 040-uniffi
...
- R5 `random_bytes::<N>() -> Result<[u8; N], CryptoError>` and `Secret::<N>::random()` MUST use `randombytes_buf` and MUST be the only randomness source in `core`.
...
- R14 The wrapper MUST reject, before any libsodium call, exactly the inputs that libsodium would abort on or reject, or that would overflow the wrapper's own arithmetic: `plaintext` of `aead_encrypt` and `secretbox_seal` whose length plus 16 does not fit in `usize` → `CryptoError::TooLong`; `buf` of `pad` whose length plus `block` does not fit in `usize` → `CryptoError::TooLong`; `block = 0` → `CryptoError::BadLength`; `password` longer than `crypto_pwhash_PASSWD_MAX` → `CryptoError::TooLong`, the length `crypto_pwhash` itself rejects with `EFBIG`. It MUST apply one guard that libsodium does not require, declared here as policy: `password` empty → `CryptoError::BadLength`, so that no key is ever derived from an empty password. Nothing else is bounded by this module: `aad`, `ciphertext`, `message`, `input` and `buf` of `stream_xor` MUST NOT be bounded here, and no shorter bound on `password` belongs here; the product's password length belongs to spec 011-config-format and every protocol or storage bound (payload, blob, frame, record, `state.bin`) to the spec of the caller.
...
| `password` | 1..=`crypto_pwhash_PASSWD_MAX` (the minimum is a wrapper guard, not a libsodium bound: no key is derived from an empty password; the product maximum is fixed by spec 011-config-format) | 0 → `BadLength`; above the maximum → `TooLong` |
...
crates/core/src/crypto/vectors.rs      loader of `010.json`, `cfg(test)`: no dependency, no I/O; spec 015-test-vectors moves it to `crates/core/src/vectors.rs`, shared by every spec
...
pub(crate) fn check_password_len(len: usize) -> Result<(), CryptoError>;                 // R14: 0 → BadLength, above crypto_pwhash_PASSWD_MAX → TooLong
...
impl<const N: usize> Secret<N> {
    ...
    pub(crate) fn copy_from(bytes: &[u8; N]) -> Self;   // added by spec 011-config-format R19, inside this impl block
...
- T03 (covers R3): `s010_t03_r03_debug_is_redacted`: `format!("{:?}")` of each entry of `SECRET_TYPES` (one `Secret<32>`, one `Secret<64>`, both non-zero) is exactly `[REDACTED]`. `s010_t04_r03_secret_has_no_forbidden_traits` reads `secret.rs` and asserts the exact derive list and the exact set of `impl` blocks, so any added trait fails the test (a `compile_fail` doctest cannot name a `pub(crate)` type; open question 010-R3).
...
- T25 (covers R16): `s010_t25_r16_manifest_pins_dependencies` reads `crates/core/Cargo.toml` and asserts exactly `libsodium-sys-stable` and `zeroize` under `[dependencies]` and `proptest` under `[dev-dependencies]`, and that `use-pkg-config`, `minimal` and `fetch-latest` are absent; `s010_t28_r16_deny_pins_the_build_script` reads `deny.toml` and asserts the `[[bans.build.bypass]]` entry for `libsodium-sys-stable`; `s010_t29_r16_libsodium_version_is_pinned` asserts `sodium_version_string()` is `1.0.22`; `cargo deny --all-features check -D checksum-mismatch` green (CI `s001_t03_r03_cargo_deny`).
...
`specs/vectors/010.json`, schema of `specs/vectors/README.md`, `proto_version = 1`. Unlike later specs, these vectors are not produced by the reference script of spec 015-test-vectors: the wrapper must match the standards, not itself. Each one declares its provenance in `source` (`published`, `derived` or `pinned`) and names it in `origin`.
...
A bump of `libsodium-sys-stable` moves three pinned values at once (the version of `s010_t29`, the build-script SHA-256 of `deny.toml` and, if the library ever changed an output, the `pinned` vectors), so the CI is the checklist and nothing has to be decided in the moment. It is a `chore:` commit and needs no ADR.

- **Version test red**: update the number. This is the expected failure of a bump, not a surprise.
- **`cargo deny` red**: the build script changed. A human reads it, checks that the archive is still verified with the minisign public key `RWQf6LRCGA9i53mlYecO4IzT51TGPpvWucNSCh1CBM0QTaLn73Y7GFO3` and that the plain-HTTP download is still only the fallback for an unreadable vendored archive, and then updates the SHA-256.
- **Any vector red**: the bump is rejected. No vector is updated, `pinned` or `published`.
- **The pull request carries the `adr-not-needed` label**, set by the same human who read the build script: the version test lives in `crates/core/src/crypto/`, a protected path, and `adr-guard` fails without it.
