...
- Text appears only in the fields `spec`, `name`, `kind`, `source`, `origin`, `error`, `content` (the outcome of a positive receive vector: `message`, `unreadable` or `stale`), `policy`, `schema` and `words` (an array of words); every other string is hexadecimal.
