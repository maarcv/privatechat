...
- R5 On a `subscribe`, the server MUST compute `relay::channel_id(pk_ch, ttl_seconds)` to name the channel in its answer. Then, in this order: a latest nonce older than 60 000 ms MUST get `error{nonce_expired, channel_id}` followed by a new `hello`; a `BadConfig` from `verify_subscribe` MUST get `error{bad_ttl, channel_id}`; a `BadSignature` under the latest nonce that arrives within 10 000 ms of the first `hello` carrying the latest nonce (a repeat does not restart that time) and verifies under the previous one (a subscribe the client released before it read the newer `hello`) MUST get `error{nonce_expired, channel_id}` followed by a `hello` that repeats the latest nonce, whose validity it does not extend; any other `BadSignature` MUST get `error{bad_auth, channel_id}`. `Ok` hands the channel to spec 030-ws-protocol R7 with `ttl_seconds` and `since`. The checks of spec 033-rate-limit-quotas R2 and R3 run before all of these, and an `Error::Internal` from any `relay` call closes the connection (spec 030-ws-protocol R16).
...
- R9 The server MUST keep, per accepted subscription, only `channel_id` and `ttl_seconds`, per connection only the latest nonce and the one before it, and MUST NOT keep `pk_ch` or `sig` past the call that checks them.

**Vectors and fuzzing**

- R10 This spec MUST add its section to `scripts/reference/vectors.py` of spec 015-test-vectors. The section writes `specs/vectors/031.json`, with `server_url` and `host` as the hex of their ASCII bytes (the text fields of spec 015 R1 are not widened): from fixed `K_ch`, TTLs, nonces and URLs, it derives `pk_ch` through the RFC 8032 §6 reference code and `crypto_kdf_derive_from_key`, computes `channel_id`, the host, the message of R2 and its signature, and writes every negative of the table "Vectors" with its error. The Rust tests of this spec reproduce the file on the server path, and those of spec 028 on the client path.
- R11 This spec MUST amend spec 016-fuzz-harness R2, R8 and R9 with the target `relay_verify_subscribe`. Its input is `pk_ch` (32) ‖ BE32(`ttl_seconds`) ‖ `sig` (64) ‖ `server_nonce` (32) ‖ host, the host truncated to 250 bytes, and `hosts` is that host followed by `example.org`; `scripts/fuzz_seeds.py` writes its corpus from `031.json` in that layout, and the nightly matrix gains its job. A proptest MUST check the round trip: for any `K_ch`, any TTL in range, any nonce and any URL of the grammar, `verify_subscribe` of the subscribe that spec 028 R6 builds returns `Ok` with the config's `channel_id`.
...
| Signature under the previous nonce | within 10 000 ms of the first `hello` of the latest nonce | `nonce_expired` and a `hello` repeating the latest nonce, not a failed attempt; later → `bad_auth` |
...
crates/server/src/auth.rs                AuthState: nonces, attempt count and deadline per connection (R1, R5–R7, R9)
crates/server/src/tests/auth.rs          s031_* server tests, over AuthState with a ManualClock and no socket
...
pub(crate) struct AuthState { nonce: [u8; 32], previous: Option<[u8; 32]>, hello_at: u64, first_hello_at: u64, failed: u8, accepted_any: bool }
pub(crate) enum Verdict { Accept { channel_id: [u8; 16] }, NonceExpired { channel_id: [u8; 16], hello: Frame },
                          BadTtl { channel_id: [u8; 16] }, BadAuth { channel_id: [u8; 16] }, Close }
impl AuthState {
    pub(crate) fn new(now: u64) -> Result<(AuthState, Frame), Error>;                       // the first hello (R1)
    pub(crate) fn on_subscribe(&mut self, pk_ch: &[u8; 32], ttl_seconds: u32, sig: &[u8; 64],
                               hosts: &[String], now: u64) -> Result<Vec<Verdict>, Error>;  // R5, R6; Close after the third failure
    pub(crate) fn deadline_passed(&self, now: u64) -> bool;                                    // R7
}
```
...
- Up to 8 hosts means up to 8 signature checks per `subscribe`, and 16 when the previous nonce is tried too; that second try exists only for 10 000 ms after each `hello`, so it cannot be repeated at will. The attempt rate of spec 033 (one per second per connection) and the close on the third failure bound that cost.
...
- `docs/spec.md` §6, brought up to date when this spec is accepted: the order of the three errors, the failed-attempt count, the check of the host against every configured URL, and the previous nonce, which within 10 000 ms of a new nonce gets `nonce_expired` and a `hello` repeating it.

## Test cases

- T01 (covers R1): `s031_t01_r01_nonce_lifetime`, over `AuthState`: the first `hello` carries 32 random bytes, and two states get different nonces; a `subscribe` signed with it 59 999 ms later → `Accept`; 60 001 ms later → `NonceExpired` naming the channel, with a `hello` of a different nonce; a `subscribe` signed with the old nonce after that → `NonceExpired` with a `hello` repeating the new nonce, and the failed count unchanged; a `subscribe` signed with the old nonce 10 001 ms after the new `hello` → `BadAuth`, and still `BadAuth` when a repeat `hello` was sent at 9 000 ms; a `subscribe` signed with a nonce older than the previous one → `BadAuth`.
...
- T05 (covers R5): `s031_t05_r05_verdicts`, over `AuthState`: a valid `subscribe` → `Accept` with the vector's `channel_id`; each negative vector → the verdict of the table "Vectors" naming the vector's `channel_id`; an expired nonce with a bad signature too → `NonceExpired` (the age is checked first).
- T06 (covers R6): `s031_t06_r06_three_failures_close`, over `AuthState`: two `BadAuth` and one `NonceExpired` → no `Close`; a third `BadAuth` → `BadAuth` then `Close`; `BadTtl`, `BadAuth`, `BadTtl` → `Close` after the third. Spec 030's socket tests check that `Close` becomes status 1008.
- T07 (covers R7): `s031_t07_r07_first_subscribe_deadline`, over `AuthState`: no `subscribe` for 60 001 ms → `deadline_passed` true; an `Accept` at 30 000 ms, then 10 minutes later → false.
- T08 (covers R8): `s031_t08_r08_hosts_from_config`: configured URLs `wss://chat.example.org` and `ws://<56 chars>.onion` → a signature over either host is accepted; over `chat.example.org:9001` → `BadAuth`; `AuthState::on_subscribe` takes no request header, so the `Host` header cannot reach it; `url_host` of `wss://h:9001` → `h`, and of `wss://h/` → `BadConfig`.
- T09 (covers R9): `s031_t09_r09_nothing_kept`: after an accepted subscribe, the `Debug` of `AuthState` and of the subscription contains neither `pk_ch` nor `sig` in hex.
