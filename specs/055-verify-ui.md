...
- R10 For a peer whose `Peer::short_collides` is true, its card MUST show the 12 words of that peer and of each other peer with the same short identifier (or of the user's own key), from the fingerprints in `context` (R1), under "Identifier identical to another member: possible impersonation. Verify by QR".
...
clients/desktop/src/lib/trust/                                          present.ts (R1), PeerCard.svelte, VerifyScreen.svelte, verify.svelte.ts
```
...
PeerView    { name: Text | Placeholder, quoted: bool, secondary: bool, verifiedIcon: bool,
              ...
              actions: [Name, Verify, Mute | Unmute, Retire, RetireOldAndName(label), Forget] }
...
```
VerifyState  = Loading | Ready { own: Fingerprint, peer: Option<PeerView> } | InPersonCheck { then } | Comparing { peer, words }
             | Scanning | NeedLabel { then } | OfferRetireOld { old, label } | Failed(kind)
VerifyIntent = CompareWords(peer) | InPersonYes | WordsMatch | WordsDiffer | ScanQr | Scanned(bytes) | LabelTyped(label) | RetireOld | Cancel
PeerIntent   = Name(label) | Verify | Mute(bool) | Retire | RetireOldAndName(label) | Forget | Regenerate
```

`Scanned` carries the bytes the scanner of spec 054-qr-invite returns, handed to `verify_scanned` and not kept. Like every scanned payload, they are zeroed after that one call whatever their content: an invitation QR scanned here by mistake holds `K_ch`.

Test names: each T below is `s055_tTT_rRR_<name>`, in snake_case in Kotlin and in camelCase after the prefix in Swift and in the desktop's Vitest (for instance `s055_t01_r01_presentationFixture`), so that `scripts/check_requirements.sh` finds each on every platform.
...
- An impostor who already holds the config can post "my new phone, please verify: word1 … word12" or a picture of their verification QR. The in-person question and the texts of R18 and R19 make that route explicit and wrong; the app cannot tell a voice call from a message, so a user who ignores them can still verify an impostor, a documented residual (`docs/spec.md` §12, "TOFU confusion").
...
None in the core. Spec 041-desktop-bridge serves `own_fingerprint`'s QR with `export_verify_qr(channel)` on its `qr` scheme (R17), and has no `verify_scanned` command (R20).
...
- T10 (covers R10): `s055_t10_r10_short_collision`: a peer with `short_collides` shows both 12-word lists from the context and the warning; a collision with the user's own key shows `own`'s words.
...
- T18 (covers R18): `s055_t18_r18_compare_words`: the in-person question comes first, and "Cancel" shows no words; two actions only; "All 12 match" on an unlabelled peer asks a label, then calls `verify`; "They differ" calls nothing and shows its text; after verifying a peer that had `claims_name_of`, the retire offer for the old key appears.
- T19 (covers R19): `s055_t19_r19_scan_verify` on Android and iOS, with a fake scanner fed the 014 vectors: the in-person question comes first; `qr_wrong_prefix` and a text of 73 bytes → "This is not a verification QR", the scanner open and no call; `qr_reference` → the label asked, `verify_scanned` with it, and the verified card with the label re-read from `peers`; `qr_other_channel` → the `WrongChannel` text; the user's own QR → the `OwnKey` text; a label the core refuses → R15's label text; the scanned bytes are zeros after the call.
- T20 (covers R20): `s055_t20_r20_desktop_no_verify_input`: the desktop verification screen has no scan action and no text input for a verification QR; the command list of spec 041-desktop-bridge has no `verify_scanned`.
